...
import { Filter, FilterTypeEnum } from 'src/modules/report/application/dto/create-report.dto';
...
export abstract class Filterable {
  ...
  protected applyFilters(
    qb: ReturnType<Repository<ProductTypeOrmEntity | InventoryTypeOrmEntity | OrderTypeOrmEntity>['createQueryBuilder']>,
    alias: string,
    filters: Filter[],
    ...
    filters.forEach((filter, index) => {
      ...
      switch (filter.type) {
        ...
        case FilterTypeEnum.RANGE:
          if (fieldType === 'Date') {
            qb.andWhere(`${fieldPath} BETWEEN :${paramKey}_min AND :${paramKey}_max`, {
