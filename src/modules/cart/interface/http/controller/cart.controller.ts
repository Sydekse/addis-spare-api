...
import { FindCartByIdDto, FindCartByUserDto } from 'src/modules/cart/application/dto/find-cart.dto';
...
import { FindCartByIdUseCase, FindCartByUserIdUseCase } from 'src/modules/cart/application/use-cases/find-cart.use-case';
...
export class CartController {
    ...
    @Post('add')
    async addToCart(@Param('id') cartID: string, @Body() cartItem: AddToCartDto): Promise<void> {
        cartItem.id = cartID; // Ensure the cart ID is set in the DTO
        return await this.addToCartUseCase.execute(cartItem);
    }
    @Delete('remove')
    async removeFromCart(@Body() removeFromCart: RemoveFromCartDto): Promise<void> {
    ...
    @Get('user/:userId')
    async findCartByUserId(@Param('userId') userId: FindCartByUserDto): Promise<any> {
